...
1. 결측치(NaN) -> 두번의 과정을 통해 해결했습니다. 첫째로 결측값의 비율을 확인하여 20%가 넘어가는 칼럼은 제거했습니다. 두번째로 NaN을 평균값으로 처리했습니다.
2. 불량률 적음 -> class_weight='balanced'를 모델링에 사용했습니다. 즉 가중치를 줌으로써 모델이 불량을 틀렸을 때 더 크게 패널티를 받게 했습니다.
3. 필요없는 칼럼 -> time을 제거했습니다
4. pass/fail -1,1 -> 0(정상), 1(불량)으로 바꿔줬습니다.
